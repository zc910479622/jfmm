    ...
    $.extend({
        ...
        table: {
            ...
            init: function (options) {
                ...
                _sidePagination = $.common.isEmpty(options.sidePagination) ? "client" : "server";
                ...
                _clickToSelect = $.common.isEmpty(options.clickToSelect) ? false : options.clickToSelect;
                _singleSelect = $.common.isEmpty(options.singleSelect) ? false : options.singleSelect;
                _onClickRow = $.common.isEmpty(options.onClickRow)? function () {}:options.onClickRow;
                _onCheckAll = $.common.isEmpty(options.onCheckAll)? function () {}:options.onCheckAll;
                _onUncheckAll = $.common.isEmpty(options.onUncheckAll)? function () {}:options.onUncheckAll;
                _onCheck = $.common.isEmpty(options.onCheck)? function () {}:options.onCheck;
                _onUncheck = $.common.isEmpty(options.onUncheck)? function () {}:options.onUncheck;
                _onLoadSuccess = $.common.isEmpty(options.onLoadSuccess)?function(){}:options.onLoadSuccess;
                _paginationShowPageGo = $.common.isEmpty(options.paginationShowPageGo) ? false : options.paginationShowPageGo;
                $('#bootstrap-table').bootstrapTable({
                    ...
                    onLoadSuccess: _onLoadSuccess,              // 表格成功加载触发事件
                    columns: options.columns,                           // 显示列信息（*）
                    clickToSelect:_clickToSelect,                       // 是否启用行选择功能
                    singleSelect:_singleSelect,
                    onClickRow:_onClickRow,                             // 行选择点击事件
                    onCheckAll:_onCheckAll,                             // 行全部选中check事件
                    onUncheckAll:_onUncheckAll,                         // 行全部取消选中check事件
                    onCheck:_onCheck,                                   // 行选中check事件
                    onUncheck:_onUncheck,                               // 行取消选中check事件
                    sidePagination: _sidePagination,                    // server启用服务端分页client客户端分页
                    search: $.common.visible(options.search),           // 是否显示搜索框功能
                    paginationShowPageGo: _paginationShowPageGo,
            ...
            selectDictLabel: function (datas, value) {
                ...
                $.each(datas, function (index, dict) {
                    if (dict.fValue == value) {
                        actions.push( dict.fLable );
                        return false;
                    }
                });
                return actions.join('');
        ...
        form: {
            ...
            reset: function (formId) {
                ...
                $("#" + currentId)[0].reset();
                $(".reset-input").val("");
                $("#" + currentId).find(".selectpicker").selectpicker('refresh');
            },
            // 获取选中复选框项
        ...
        modal: {
            ...
            open2: function (title, url, width, height, callback,cancelCallBack) {
                //如果是移动端，就使用自适应大小弹窗
                if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
                    width = 'auto';
                    height = 'auto';
                ...
                if ($.common.isEmpty(cancelCallBack)) {
                    cancelCallBack = function (index, layero) {
                        var iframeWin = layero.find('iframe')[0];
                        iframeWin.contentWindow.cancelCallBack();
                    }
                }
        ...
        operate: {
            ...
            add: function (id,width) {
                ...
                //     width: (width==null||width.length<=0)?"800":width,
                //     url: url
                // }
                // $.modal.openOptions(options);
                $.modal.open("添加" + $.table._option.modalName, url,width);

            },
            // 修改信息
            edit: function (id,width,isEdit) {
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
                    url = $.table._option.updateUrl.replace("{id}", id.replace(/\//g,"%7C"));
                } else {
                ...
                // }
                // $.modal.openOptions(options);
                $.modal.open((isEdit==null?"修改":"") + $.table._option.modalName, url,width);
            },
            edit2: function (id,width,isEdit) {
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
                    url = $.table._option.updateUrl.replace("{id}", id.replace(/\//g,"%7C"));
                } else {
                ...
                // }
                // $.modal.openOptions(options);
                $.modal.open2((isEdit==null?"修改":"") + $.table._option.modalName, url,width);
            },
            //接收部件流转
            receive: function (id,width) {
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
            ...
            successCallback: function (result) {
                if (result.code == web_status.SUCCESS) {
                    ...
                    } else if (window.parent.$("#bootstrap-tree").length > 0) {
                        $.modal.close();
        ...
        Switch: {
            setSwitch: function (id, state) {
                $("#" + id).bootstrapSwitch("state", state);
            }
        },
        TimeTool:{
            /**************日期时间转换插件***************/
            timestampTime: function (str, index) {
                if (str != null) {
