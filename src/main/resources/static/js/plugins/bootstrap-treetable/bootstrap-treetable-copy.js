...
(function($) {
    ...
    var bootstrapTreeTableCopy = function(el, options) {
    ...
    // 初始化
    bootstrapTreeTableCopy.prototype.init = function(parms) {
        // 初始化配置
        this.initOptions();
        // 初始化容器
        this.initContainer();
    ...
    // 初始化配置
    bootstrapTreeTableCopy.prototype.initOptions = function() {
        var self = this;
        $.each(self.options.columns, function(i, column) {
            column = $.extend({}, bootstrapTreeTableCopy.COLUMN_DEFAULTS, column);
            if(column.width){
    ...
    // 初始化容器
    bootstrapTreeTableCopy.prototype.initContainer = function() {
        var self = this;
    ...
    // 初始化工具栏
    bootstrapTreeTableCopy.prototype.initToolbar = function() {
        var self = this;
    ...
    // 初始化隐藏列
    bootstrapTreeTableCopy.prototype.initHiddenColumns = function() {
        var self = this;
    ...
    // 初始化表头
    bootstrapTreeTableCopy.prototype.initHeader = function() {
        var self = this;
        var $thr = $('<tr></tr>');
        // 因有可能出现冻结列，所以这里合并列配置
        ...
        self.$el.find('.treetable-thead').html("");
        // header跟着滚动
    ...
    // 初始化表体
    bootstrapTreeTableCopy.prototype.initBody = function() {
        var self = this;
        var $tbody = $('<tbody class="treetable-tbody"></tbody>');
        self.$el.append($tbody);
    };
    // 初始化数据服务
    bootstrapTreeTableCopy.prototype.initServer = function(parms) {
        var self = this;
        // 加载数据前先清空
        self.data_list = {};
        self.data_obj = {};
    ...
    // 加载完数据后渲染表格
    bootstrapTreeTableCopy.prototype.renderTable = function(data) {
        var self = this;
        var $tbody = self.$el.find("tbody");
        // 先清空
        $tbody.html("");
    ...
    // 动态设置表头宽度及表体偏移量
    bootstrapTreeTableCopy.prototype.autoReSize = function() {
        var self = this;
        // 原表的thead
    ...
    // 固定列设置
    bootstrapTreeTableCopy.prototype.frozen = function() {
        var self = this;
        // 先清掉
        if(self.$leftBox){
            self.$leftBox.remove();
    ...
    bootstrapTreeTableCopy.prototype.cloneTable = function($table,$newTable,iColumnStart,iColumnEnd) {
        var self = this;
        // 克隆 table 属性
    ...
    // 合并属性
    bootstrapTreeTableCopy.prototype.mergeAttributes = function($s,$t) {
    ...
    // 缓存并格式化数据
    bootstrapTreeTableCopy.prototype.formatData = function(data) {
        var self = this;
    ...
    // 递归获取子节点并且设置子节点
    bootstrapTreeTableCopy.prototype.recursionNode = function(parentNode, lv, row_id, p_id) {
        var self = this;
        var $tbody = self.$el.find("tbody");
    ...
    // 绘制行
    bootstrapTreeTableCopy.prototype.renderRow = function(item, isP, lv, row_id, p_id) {
        var self = this;
        // 标记已显示
        item.isShow = true;
        item.row_id = row_id;
    ...
    bootstrapTreeTableCopy.prototype.hasScroll = function(flag){
        var self = this;
    ...
    //获取滚动条宽度
    bootstrapTreeTableCopy.prototype.getScrollWidth = function(elem){
        var width = 0;
        if(elem){
    ...
    // 注册刷新按钮点击事件
    bootstrapTreeTableCopy.prototype.registerRefreshBtnClickEvent = function() {
        var self = this;
    ...
    // 注册列选项事件
    bootstrapTreeTableCopy.prototype.registerColumnClickEvent = function() {
        var self = this;
    ...
    // 注册行点击事件
    bootstrapTreeTableCopy.prototype.registerRowEvent = function() {
        var self = this;
    ...
    // 选择列管理者
    bootstrapTreeTableCopy.prototype.selectedDataIdsManager = function(itemVal,isCheckbox,isAdd) {
        var self = this;
    ...
    // 注册小图标点击事件--展开缩起
    bootstrapTreeTableCopy.prototype.registerExpanderEvent = function() {
        var self = this;
    ...
    // 刷新数据
    bootstrapTreeTableCopy.prototype.refresh = function(parms) {
        var self = this;
        self.destroy();
        if (parms) {
    ...
    // 添加数据刷新表格
    bootstrapTreeTableCopy.prototype.appendData = function(data) {
        var self = this;
        $.each(data, function(i, item) {
    ...
    // 展开/折叠指定的行
    bootstrapTreeTableCopy.prototype.toggleRow = function(id) {
        var self = this;
    ...
    // 展开指定的行
    bootstrapTreeTableCopy.prototype.expandRow = function(id) {
        var self = this;
        var _rowData = self.data_obj["id_" + id];
        var $tr = self.$el.find("tr[rid='" + _rowData.row_id+ "']");
        var $row_expander = $tr.find(".treetable-expander");
    ...
    // 折叠 指定的行
    bootstrapTreeTableCopy.prototype.collapseRow = function(id) {
        var self = this;
        var _rowData = self.data_obj["id_" + id];
        var $tr = self.$el.find("tr[rid='" + _rowData.row_id+ "']");
        var $row_expander = $tr.find(".treetable-expander");
    ...
    // 展开所有的行
    bootstrapTreeTableCopy.prototype.expandAll = function() {
        var self = this;
        if(self.hasFixedColumn && self.expandColumnIsFixed){
    ...
    // 折叠所有的行
    bootstrapTreeTableCopy.prototype.collapseAll = function() {
        var self = this;
        if(self.hasFixedColumn && self.expandColumnIsFixed){
            self.$leftBox.find("tbody").find("tr").find(".treetable-expander").each(function(i, n) {
    ...
    // 显示指定列
    bootstrapTreeTableCopy.prototype.showColumn = function(field) {
        var self = this;
        // 固定列不让隐藏
        $.each(self.leftFixedColumns, function(i, column) {
    ...
    // 隐藏指定列
    bootstrapTreeTableCopy.prototype.hideColumn = function(field) {
        var self = this;
        // 固定列不让隐藏
        $.each(self.leftFixedColumns, function(i, column) {
            if (column.field == field) {
    ...
    // 获取已选行
    bootstrapTreeTableCopy.prototype.getSelections = function() {
        var self = this;
    ...
    // 触发事件
    bootstrapTreeTableCopy.prototype.trigger = function(name) {
        var self = this;
        var args = Array.prototype.slice.call(arguments, 1);

        name += '.bs.tree.table';
        self.options[bootstrapTreeTableCopy.EVENTS[name]].apply(self.options, args);
    ...
    // 销毁
    bootstrapTreeTableCopy.prototype.destroy = function() {
        var self = this;
    ...
    // 组件方法
    bootstrapTreeTableCopy.METHODS = [
        "getSelections",
        "refresh",
        "appendData",
        "toggleRow",
    ...
    // 默认配置
    bootstrapTreeTableCopy.DEFAULTS = {
    ...
    bootstrapTreeTableCopy.COLUMN_DEFAULTS = {
        radio: false,
        checkbox: false,
        field: undefined,
        title: undefined,
    ...
    $.fn.bootstrapTreeTableCopy = function(option) {
        var value,
            args = Array.prototype.slice.call(arguments, 1);
        this.each(function() {
            var $this = $(this),
                data = $this.data('bootstrap.tree.table'),
                options = $.extend({}, bootstrapTreeTableCopy.DEFAULTS, $this.data(), typeof option === 'object' && option);
            if (typeof option === 'string') {
                if ($.inArray(option, bootstrapTreeTableCopy.METHODS) < 0) {
            ...
            if (!data) {
                $this.data('bootstrap.tree.table', (data = new bootstrapTreeTableCopy(this, options)));
            }
        });
